...
The single Brand-Fidelity scorer is decomposed into a panel of FOUR Gemini Flash
seats, each judging one lens of "on-brand + cleared":

  * **brand**      — brand-consistency: palette / lockups / grid vs. the asset bank
  * **voice**      — voice-tone: calm, candid, anti-hype vs. the canon's voice rules
...
1-decimal precision) folds the four sub-scores into the ONE ``FIDELITY_SCORE`` the
loop's deterministic checker reads. The model's four numbers are *reported*; the

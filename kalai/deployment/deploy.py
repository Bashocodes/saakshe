...
"""Deployment script for kalai (MAKES) — Vertex AI Agent Engine.

Packages the full ADK studio pipeline (Claude Creative Director → ParallelAgent
Designer + Copy → Brand-Fidelity LoopAgent → Claude fail-closed Compliance gate)
and pushes it to a managed Agent Engine with tracing on. kalai holds no channel
keys and never publishes — its only world-facing act is token spend.

Run from the saakshe project root (PYTHONPATH=. so `kalai` and `common` import):

    PYTHONPATH=. python kalai/deployment/deploy.py --create
    PYTHONPATH=. python kalai/deployment/deploy.py --list
    PYTHONPATH=. python kalai/deployment/deploy.py --delete --resource_id <id>
"""
...
from kalai.agent import root_agent

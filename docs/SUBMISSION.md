...
**Tagline (85 chars):**
One founder, a whole company, one witness. The staff works; you approve what matters.
...
- ▲ **kalai (makes)** — all media and every word of copy. Stills via Nano Banana Pro — Gemini 3 Pro Image on Vertex, with a Nano Banana 2 fallback, the same pair our production platform aikizi runs (the Veo reel wrapper is shipped; wiring it into the flywheel is roadmap).
...
3. **The founder approves the decision** at arivu's gate. Nothing acts before this — and when a verdict is defensible but close, the chamber raises a signed question to the founder instead of pretending certainty.
4. **kalai makes the announcement.** Creative brief through its own chamber (deciding factor: brand fit), copy in the founder's extracted voice, the still rendered with Nano Banana Pro, brand assets pulled from the vault that was auto-extracted at connect. Every variant is verified against the brand block before it clears.
...
6. **The founder approves the publish** at kural's gate. The executor fires only after approval, and dry-run is the hardcoded default.
...
At every step the pattern is the same: an agent **acts**, then something **verifies** — a citation check, a prosecutor, a schema, founder approval. Errors never sink the day (a failed render or connect degrades soft); safety gates fail closed.
...
**Three modes, honestly separated.** `demo` runs the entire orchestration creds-free with scripted model output — deterministic and byte-identical, so judges and CI see the same day. `hybrid` runs every seat live — the Claude seats on a live Gemini understudy until our Model Garden quota clears. `live` is all real, including Nano Banana Pro still renders and the witness's Gemini Live voice.
...
- **kalai's hands are proven live.** A real Vertex still render, generated from a brand-grounded prompt, is checked into the repo at `docs/first_creation.png`.
...
Google ADK, Vertex AI, Gemini, Claude (via Vertex Model Garden), Nano Banana Pro (Gemini 3 Pro Image), Veo, Cloud Run, FastAPI, Supabase, OpenTelemetry, Python

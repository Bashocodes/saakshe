...
if os.environ.get("SAAKSHE_ALLOW_LIVE_SEND") == "1":
    try:
        # The channel KEY lives in manas (the keeper). kural holds only a tokenless
        # capability handle that dispatches to the broker; set_channel_client stays
        # HERE on kural so has_channel_client() and the orchestrator arm-gate are
        # unchanged. The token never reaches kural.
        ...
        if _connectors.channel_configured():
            ...
            print("kural: live channel armed → manas connector (key custody)")
